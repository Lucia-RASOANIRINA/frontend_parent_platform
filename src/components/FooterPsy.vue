<template>
  <footer class="relative bg-gradient-to-b from-[#0F172A] via-[#0F2942] to-[#6F4E37] text-white mt-auto overflow-hidden">
    ...
    <div class="absolute inset-0 opacity-20 pointer-events-none">
      ...
      <div class="absolute bottom-0 right-0 w-80 h-80 bg-[#D2B48C] rounded-full filter blur-[100px] animate-pulse-slow" style="animation-delay: 2s;"></div>
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#5D4037] rounded-full filter blur-[150px] opacity-30"></div>
    </div>
    ...
    <div class="absolute top-0 left-0 w-full overflow-hidden leading-0 rotate-180">
      <svg class="relative block w-full h-10" viewBox="0 0 1200 120" preserveAspectRatio="none">
        <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z"
          fill="#FAF7F2" fill-opacity="0.08"></path>
      </svg>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-8 mb-10">
        ...
        <div class="md:col-span-4 space-y-4">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h2 class="text-2xl font-bold tracking-tight text-white">
                Cabinet<span class="text-[#D2B48C]">'Psy</span>
              </h2>
          ...
          <div class="inline-flex items-center gap-2 bg-white/5 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/10">
            <svg class="w-3 h-3 text-[#D2B48C]" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="md:col-span-3">
          <h3 class="text-white font-bold text-base mb-5 relative inline-block">
            Navigation
            <div class="absolute -bottom-2 left-0 w-full h-[2px] bg-gradient-to-r from-[#D2B48C] to-transparent rounded-full"></div>
          </h3>
          <ul class="space-y-3">
            <li v-for="link in navLinks" :key="link.name">
              ...
              >
                <span class="w-5 h-5 flex items-center justify-center rounded-lg bg-white/5 group-hover:bg-white/10 transition-all">
                  <component :is="link.icon" class="w-3 h-3 text-[#D2B48C] group-hover:scale-110 transition-transform" />
                </span>
          ...
          <ul class="space-y-3">
            <li>
              <router-link 
                ...
                class="text-white/50 hover:text-white transition-all duration-300 text-sm flex items-center gap-3 group"
              >
                <svg class="w-4 h-4 text-[#D2B48C] group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="md:col-span-3">
          ...
          <ul class="space-y-4">
            <li>
              ...
              >
                <svg class="w-4 h-4 text-[#D2B48C] group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li class="flex items-center gap-3 text-white/50 text-sm">
              <svg class="w-4 h-4 text-[#D2B48C]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li class="flex items-start gap-3 text-white/50 text-sm">
              <svg class="w-4 h-4 text-[#D2B48C] mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="absolute bottom-0 left-0 w-full overflow-hidden leading-0">
      <svg class="relative block w-full h-6" viewBox="0 0 1200 120" preserveAspectRatio="none">
        <path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" 
          fill="#FAF7F2" fill-opacity="0.05"></path>
      </svg>
    </div>

    <!-- MODALE DE CONFIRMATION DÉCONNEXION (identique à celle du header) -->
      ...
      <div v-if="showLogoutConfirm" class="fixed inset-0 flex items-center justify-center z-[100] px-4">
        ...
        <div class="bg-white rounded-[3rem] p-10 max-w-sm w-full relative shadow-2xl text-center border border-[#FAF7F2]">
          ...
          <div class="flex flex-col space-y-4">
            <button @click="confirmLogout" class="w-full py-4 rounded-2xl bg-[#6F4E37] text-white text-xs font-black uppercase tracking-[0.2em] hover:bg-[#3C2A21] transition-all shadow-lg active:scale-95">Terminer la session</button>
            <button @click="closeLogoutModal" class="group flex items-center justify-center w-full py-2 text-[#64748B] text-[10px] font-bold uppercase tracking-widest transition-all">
              ...
              <span class="group-hover:text-[#6F4E37] transition-colors">Rester connecté</span>
            </button>
          </div>
        </div>
      </div>
