  ...
  >
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
      <div class="flex justify-between items-center h-20">
        ...
        <router-link to="/admin" class="flex items-center space-x-3 group shrink-0">
        ...
        <div class="flex items-center space-x-3 shrink-0">
          ...
          <div class="relative menu-compte">
            <button @click="menuOpen = !menuOpen" :aria-expanded="menuOpen" :aria-label="t('entete.menuCompte')"
                    class="focus:outline-none group p-1 rounded-full hover:bg-slate-50 transition-all">
            ...
            <transition name="pop-in">
              <div v-if="menuOpen" class="absolute right-0 mt-4 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50">
                ...
                <p class="px-5 pt-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ t('entete.visiterEnTantQue') }}</p>
          ...
          <button @click="mobileMenuOpen = !mobileMenuOpen" class="xl:hidden text-slate-800 p-2 hover:bg-slate-50 rounded-full transition-colors relative z-50">
      ...
      <transition name="mobile-slide">
        <div v-if="mobileMenuOpen" class="xl:hidden absolute top-24 left-4 right-4 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-6 z-40">
          <div class="space-y-2">
...
const viewRoles = computed(() => [
  { role: 'PARENT', label: t('roles.parent'), short: 'P', color: '#6F4E37' },
  { role: 'EDUCATEUR', label: t('roles.educatrice'), short: 'É', color: '#C19A6B' },
  { role: 'PSY', label: t('roles.psychologue'), short: 'Ψ', color: '#8B7355' },
])
...
const navLinks = computed(() => [
  { name: t('nav.tableauBord'), path: '/admin', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>' },
  { name: t('nav.utilisateurs'), path: '/admin/utilisateurs', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>' },
  { name: t('nav.evenements'), path: '/admin/evenements', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>' },
  { name: t('nav.conferences'), path: '/admin/conferences', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>' },
  { name: t('nav.contenu'), path: '/admin/contenu', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>' }
])
