<template>
  <header 
    ...
    :class="{ 'shadow-sm border-b border-[#FAF7F2]': isScrolled }"
  >
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
      <div class="flex justify-between items-center h-20">
        
        <router-link to="/hometeacher" class="flex items-center space-x-3 group shrink-0">
        ...
        <div class="hidden xl:flex items-center gap-5 2xl:gap-7 min-w-0">
          <router-link 
            ...
            :key="link.path" 
            :to="link.path" 
            class="relative py-2 flex items-center gap-2 text-[#3C2A21] transition-all duration-300 group shrink-0"
            v-slot="{ isActive }"
          >
            <span class="w-4 h-4 text-[#C19A6B] group-hover:text-[#6F4E37] transition-colors" v-html="link.icon"></span>
            
            <div class="flex items-center gap-1.5">
              <span class="text-[11px] font-black uppercase tracking-[0.08em] whitespace-nowrap group-hover:text-[#6F4E37] transition-colors">
                {{ link.name }}
              </span>
              
              <transition name="star-fade">
                <svg v-if="isActive" class="w-3.5 h-3.5 text-[#6F4E37]" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center space-x-3 shrink-0">
          ...
          <div class="relative menu-compte">
            <button @click="toggleMenu" :aria-expanded="menuOpen" :aria-label="t('entete.menuCompte')"
              ...
              <div class="w-10 h-10 rounded-full bg-gradient-to-br from-[#6F4E37] to-[#C19A6B] flex items-center justify-center text-white font-bold text-base shadow-sm ring-2 ring-transparent group-hover:ring-[#F5F5DC]">
                {{ userInitial }}
              </div>
            </button>
            
            <transition name="pop-in">
              <div v-if="menuOpen" class="absolute right-0 mt-4 w-64 bg-white rounded-2xl shadow-xl border border-[#F5F5DC] py-2 z-50">
                <div class="px-5 py-3 border-b border-[#FAF7F2] mb-1">
                  <p class="text-sm font-semibold text-[#3C2A21] truncate">{{ user.nom || 'Éducatrice' }}</p>
                </div>
                
                <router-link to="/profilteacher" class="group flex items-center px-5 py-3 text-[#3C2A21] hover:bg-[#FAF7F2] transition-colors">
                ...
                <button @click="openLogoutModal" class="group flex items-center w-full px-5 py-3 text-[#A0522D] hover:bg-[#FAF7F2] transition-colors">
                  <span class="w-8 h-8 rounded-lg bg-[#FAF7F2] flex items-center justify-center mr-3 group-hover:bg-white transition-colors shadow-sm">
          ...
          <button @click="mobileMenuOpen = !mobileMenuOpen" class="xl:hidden text-[#3C2A21] p-2 hover:bg-[#FAF7F2] rounded-full transition-colors relative z-50">
        ...
        <div v-if="mobileMenuOpen" class="xl:hidden absolute top-24 left-4 right-4 bg-white rounded-[2.5rem] shadow-2xl border border-[#F5F5DC] p-6 z-40">
          <div class="space-y-2">
            <router-link v-for="link in navLinks" :key="link.path" :to="link.path" @click="mobileMenuOpen = false"
              class="flex items-center justify-between px-6 py-4 text-[#3C2A21] font-bold uppercase text-xs tracking-widest hover:bg-[#FAF7F2] rounded-2xl transition-all"
              v-slot="{ isActive }"
            >
      ...
      <div v-if="showLogoutConfirm" class="fixed inset-0 flex items-center justify-center z-[100] px-4">
        ...
        <div class="bg-white rounded-[3rem] p-10 max-w-sm w-full relative shadow-2xl text-center border border-[#F5F5DC]">
          <div class="mx-auto w-20 h-20 rounded-full bg-[#FAF7F2] flex items-center justify-center mb-8 border-4 border-white shadow-md">
            <svg class="w-10 h-10 text-[#6F4E37]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>
          </div>
          <h3 class="text-3xl font-extralight text-[#3C2A21] mb-4">{{ t('entete.aBientot') }}</h3>
          <p class="text-[#8a6d4b] text-sm leading-relaxed mb-10 italic">{{ t('entete.citationEduc') }}</p>
          <div class="flex flex-col space-y-4">
            <button @click="confirmLogout" class="w-full py-4 rounded-2xl bg-[#6F4E37] text-white text-xs font-black uppercase tracking-[0.2em] hover:bg-[#3C2A21] transition-all shadow-lg active:scale-95">{{ t('entete.terminerJournee') }}</button>
            <button @click="closeLogoutModal" class="group flex items-center justify-center w-full py-2 text-[#8a6d4b] text-[10px] font-bold uppercase tracking-widest transition-all">
              ...
              <span class="group-hover:text-[#6F4E37] transition-colors">{{ t('entete.continuerAtelier') }}</span>
            </button>
          </div>
        </div>
      </div>
...
const navLinks = computed(() => [
  { name: t('nav.actu'), path: '/hometeacher', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v16m-6 0a2 2 0 002 2h2a2 2 0 002-2"></path></svg>' },
  { name: t('nav.mesPublications'), path: '/teacherpub', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>' },
  { name: t('nav.ressources'), path: '/ressourcesteacher', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>' },
  { name: t('nav.evenements'), path: '/teacher/evenements', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>' },
  { name: t('nav.conferences'), path: '/teacher/conferences', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>' },
  { name: t('nav.discussions'), path: '/discussionsteacher', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>' }
])
