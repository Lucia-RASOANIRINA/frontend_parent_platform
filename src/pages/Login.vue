        ...
        <div class="message-box" :class="messageBox.type" :style="getMessageBoxStyle()">
          ...
          <div class="message-box-inner">
            <div class="message-box-icon" :style="getMessageIconStyle()">
              ...
              <svg v-else-if="messageBox.type === 'welcome-admin' || messageBox.type.toLowerCase().includes('admin')" class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14.5c-1.5 0-3-1-3-3s1.5-3 3-3 3 1 3 3-1.5 3-3 3zm0 0v4m-6-4h12M9 21h6" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v2m-4 2H6m12 0h-2" />
              </svg>
        ...
        <div class="flex-grow p-8 pt-2 relative overflow-hidden">
          <Transition 
            ...
            <form v-else key="register" @submit.prevent="handleRegister" class="flex flex-col h-full gap-3.5">
              ...
              <div class="relative">
                <div class="grid grid-cols-4 gap-1 p-1 bg-[#EFE9E4]/30 rounded-2xl border border-[#D2B48C]/20">
                  <button
                    type="button"
                    @click="registerForm.role = 'PARENT'"
                    class="flex-1 py-2 rounded-xl transition-all duration-300 relative overflow-hidden group"
                    ...
                    <div class="flex items-center justify-center gap-1">
                      ...
                      <span class="text-[11px] font-semibold">Parent</span>
                    </div>
                  </button>
                  <button
                    type="button"
                    ...
                    <div class="flex items-center justify-center gap-1">
                      ...
                      <span class="text-[11px] font-semibold">Éduc.</span>
                    </div>
                  </button>
                  <button
                    type="button"
                    ...
                    <div class="flex items-center justify-center gap-1">
                      ...
                      <span class="text-[11px] font-semibold">Psy</span>
                    </div>
                  </button>
                  <button
                    type="button"
                    ...
                    class="flex-1 py-2 rounded-xl transition-all duration-300 relative overflow-hidden group"
                    :class="registerForm.role === 'ADMIN' ? 'bg-gradient-to-r from-[#3E2C1F] to-[#2A1F14] text-white shadow-lg' : 'text-white/70 hover:bg-white/10'">
                    <div class="flex items-center justify-center gap-1">
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14.5c-1.5 0-3-1-3-3s1.5-3 3-3 3 1 3 3-1.5 3-3 3zm0 0v4m-6-4h12M9 21h6" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v2m-4 2H6m12 0h-2" />
...
const getRoleSpecificMessage = (role: string, userName: string) => {
  ...
  const messages = {
    ...
    ADMIN: { title: 'Bienvenue Administrateur', text: `Bienvenue ${userName}, vous avez les droits d'administration. Accédez au tableau de bord.`, url: '/HomeAdmin' }
  }
...
const handleRegister = async () => {
  ...
  try {
    ...
    showMessageBox('success', 'Inscription réussie', roleMessages[registerForm.role])
