<template>
  <div class="w-full font-sans antialiased bg-white min-h-screen">
    ...
    <transition name="notify-slow">
      <div v-if="alert.show" 
        class="fixed top-6 right-4 left-4 md:left-auto md:right-8 md:w-80 z-[100] flex items-center gap-4 px-5 py-4 rounded-2xl shadow-2xl border border-[#FAF7F2] bg-white/95 backdrop-blur-md"
      >
        ...
        <div class="flex-grow">
          ...
          <p class="text-xs font-bold text-[#0F172A]">{{ alert.message }}</p>
        </div>
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-shrink-0">
          <div 
            ...
            class="w-12 h-12 rounded-2xl bg-[#FAF7F2] border border-[#FAF7F2] flex items-center justify-center text-[#0F172A] font-black text-sm shadow-sm transition-all duration-500 hover:border-[#6F4E37] hover:scale-105 cursor-pointer"
          >
            {{ userInitials }}
          </div>
        </div>
        ...
        <div class="flex-grow">
          <div class="relative group border-b border-[#FAF7F2] transition-all duration-500" :class="{ 'border-[#6F4E37]': isExpanded }">
            <textarea 
              v-model="formData.contenu" 
              @focus="isExpanded = true"
              placeholder="Partagez quelque chose..." 
              class="w-full bg-transparent border-none text-base md:text-lg font-medium text-[#0F172A] placeholder-gray-300 outline-none resize-none transition-all py-2"
              :class="isExpanded ? 'h-24' : 'h-10'"
            ></textarea>

            <!-- Aperçu des fichiers sélectionnés -->
            <div v-if="imagePreviewUrl || pdfFile" class="mt-2 p-2 bg-[#FAF7F2] rounded-lg flex items-center gap-3">
              <div v-if="imagePreviewUrl" class="relative">
                <img :src="imagePreviewUrl" class="h-16 w-16 object-cover rounded-lg border border-[#FAF7F2]">
              ...
              <div v-if="pdfFile" class="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-[#FAF7F2]">
              ...
              <div v-if="isExpanded" class="flex items-center justify-between py-4">
                ...
                <div class="relative">
                  <div class="md:hidden">
                    <button @click="showUploadMenu = !showUploadMenu" 
                      class="p-2 rounded-full bg-[#FAF7F2] text-[#0F172A] transition-all"
                      :class="{ 'rotate-45 bg-[#0F172A] text-white': showUploadMenu }"
                    >
                      <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" stroke-width="2.5"/></svg>
                    </button>
                    <transition name="pop">
                      <div v-if="showUploadMenu" class="absolute bottom-full left-0 mb-3 bg-white border border-[#FAF7F2] rounded-2xl shadow-xl p-2 z-50 min-w-[140px]">
                        ...
                        <button v-if="userRole !== 'parent'" @click="triggerInput('file')" class="flex items-center gap-3 w-full px-3 py-2 hover:bg-[#FAF7F2] rounded-xl transition-colors border-t border-[#FAF7F2]">
                  ...
                  <div class="hidden md:flex items-center gap-3">
                    <button @click="triggerInput('image')" class="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FAF7F2] text-[#6F4E37] hover:bg-[#FAF7F2] transition-all border border-[#FAF7F2] group">
                    ...
                    <button v-if="userRole !== 'parent'" @click="triggerInput('file')" class="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FAF7F2] text-[#6F4E37] hover:bg-[#FAF7F2] transition-all border border-[#FAF7F2] group">
                ...
                <div class="flex items-center gap-5">
                  ...
                  >
                    ...
                    <div class="absolute inset-0 bg-[#5D4037] translate-y-full transition-transform duration-500 ease-out btn-overlay"></div>
                  </button>
                </div>
              </div>
            </transition>
    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
      ...
      <div v-if="isLoadingPosts" class="space-y-6">
        <div v-for="i in 3" :key="i" class="bg-white rounded-2xl border border-[#FAF7F2] overflow-hidden shadow-sm p-4 animate-pulse">
          <div class="flex gap-3">
            <div class="w-10 h-10 rounded-full bg-[#FAF7F2]"></div>
            <div class="flex-1">
              <div class="h-4 bg-[#FAF7F2] rounded w-32 mb-2"></div>
              <div class="h-3 bg-[#FAF7F2] rounded w-24"></div>
            </div>
          </div>
          <div class="mt-4 space-y-2">
            <div class="h-3 bg-[#FAF7F2] rounded w-full"></div>
            <div class="h-3 bg-[#FAF7F2] rounded w-5/6"></div>
          </div>
          <div class="mt-4 h-40 bg-[#FAF7F2] rounded-lg"></div>
        </div>
      </div>

      <!-- Aucun post -->
      <div v-else-if="posts.length === 0" class="text-center py-12 bg-white rounded-2xl border border-[#FAF7F2]">
      ...
      <div v-else class="space-y-6">
        <div v-for="post in posts" :key="post.id" class="bg-white rounded-2xl border border-[#FAF7F2] overflow-hidden shadow-sm hover:shadow-md transition-all">
          ...
          <div class="p-4 flex justify-between items-start">
            <div class="flex gap-3">
              <div 
                ...
                class="w-10 h-10 rounded-full bg-gradient-to-br from-[#6F4E37] to-[#5D4037] flex items-center justify-center text-white font-bold cursor-pointer hover:scale-105 transition-transform"
              >
                {{ (post.user?.nom?.charAt(0) || 'U').toUpperCase() }}
              </div>
              <div>
                <div class="font-bold text-[#0F172A] text-sm">{{ post.user?.nom || 'Utilisateur inconnu' }}</div>
            ...
            <button 
              ...
              class="flex items-center gap-2 px-3 py-1.5 bg-[#FAF7F2] rounded-full text-xs text-[#0F172A] hover:bg-[#6F4E37] hover:text-white transition-all"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="post.fileData" class="px-4 py-2">
            <button @click="confirmDownload(post)" class="inline-flex items-center gap-2 text-sm text-[#6F4E37] hover:text-[#5D4037] transition-colors">
          ...
          <div class="px-4 py-2 flex items-center gap-4 border-t border-[#FAF7F2]">
            <button @click="toggleLike(post.id)" class="flex items-center gap-2 px-2 py-1.5 hover:bg-[#FAF7F2] rounded-lg transition-all" :class="post.liked ? 'text-red-500' : 'text-gray-500'">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="activeComments === post.id" class="border-t border-[#FAF7F2] p-4 bg-[#FAFAFA]">
            <div class="space-y-3 max-h-60 overflow-y-auto">
              <div v-for="comment in post.comments" :key="comment.id" class="flex gap-2 text-sm">
                <span class="font-bold text-[#0F172A]">{{ comment.user?.nom || 'Utilisateur' }}:</span>
            ...
            <div class="flex gap-2 mt-3">
              <input v-model="newComment[post.id]" type="text" placeholder="Écrire un commentaire..." 
                class="flex-grow bg-white border border-[#FAF7F2] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#6F4E37]">
              <button @click="addComment(post.id)" class="bg-[#6F4E37] text-white px-4 py-2 rounded-lg text-sm hover:bg-[#5D4037] transition-colors">
                Envoyer
              </button>
            </div>
          </div>
      ...
      <transition name="fade-scale">
        <div v-if="imageViewer.show" class="fixed inset-0 bg-black/80 flex items-center justify-center z-[200] p-4" @click.self="closeImageViewer">
          <div class="relative max-w-4xl max-h-[90vh] bg-white rounded-xl overflow-hidden">
            ...
            <button @click="downloadCurrentImage" class="absolute bottom-4 right-4 bg-[#0F172A] text-white p-2 rounded-full hover:bg-[#6F4E37] transition-colors">
        ...
        <div v-if="confirmModal.show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-[200] p-4" @click.self="closeConfirmModal">
          <div class="bg-white rounded-xl max-w-sm w-full p-6 shadow-2xl text-center">
            <div class="flex justify-center mb-4">
              <div class="w-16 h-16 rounded-full bg-[#FAF7F2] flex items-center justify-center">
                <svg class="w-8 h-8 text-[#6F4E37]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                </svg>
              </div>
            </div>
            <h3 class="text-lg font-bold text-[#0F172A] mb-2">{{ confirmModal.title }}</h3>
            ...
            <div class="flex justify-center gap-3">
              ...
              <button @click="confirmModal.onConfirm" class="px-5 py-2 text-sm font-medium text-white bg-[#6F4E37] rounded-lg hover:bg-[#5D4037] transition-colors">
                Confirmer
              </button>
            </div>
          </div>
...
/* Carte utilisateur (version bleue) */
.user-info-card {
  position: relative;
  width: 280px;
  background: white;
  border-radius: 1.25rem;
  box-shadow: 0 20px 35px -12px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(207, 250, 254, 0.5);
  overflow: hidden;
}
...
.user-info-arrow {
  ...
  height: 14px;
  background: white;
  border-left: 1px solid rgba(207, 250, 254, 0.8);
  border-top: 1px solid rgba(207, 250, 254, 0.8);
