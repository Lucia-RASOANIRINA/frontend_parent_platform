<template>
  <div class="w-full font-sans antialiased bg-white min-h-screen">
              ...
              <div v-if="isExpanded" class="flex items-center justify-between py-4">
                ...
                <div class="relative">
                  <div class="md:hidden">
                    ...
                    <transition name="pop">
                      <div v-if="showUploadMenu" class="absolute bottom-full left-0 mb-3 bg-white border border-[#F3EEEA] rounded-2xl shadow-xl p-2 z-50 min-w-[140px]">
                        ...
                        <button v-if="userRole !== 'parent'" @click="triggerInput('file')" class="flex items-center gap-3 w-full px-3 py-2 hover:bg-coffee-50 rounded-xl transition-colors border-t border-[#FDFBF9]">
                          <svg class="w-4 h-4 text-coffee-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" stroke-width="2"/></svg>
                  ...
                  <div class="hidden md:flex items-center gap-3">
                    ...
                    <button v-if="userRole !== 'parent'" @click="triggerInput('file')" class="flex items-center gap-2 px-4 py-1.5 rounded-full bg-coffee-50 text-coffee-700 hover:bg-coffee-100 transition-all border border-coffee-100/50 group">
    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
      ...
      <!-- Loading -->
      <Chargement v-if="isLoadingPosts" />
...
const fetchPosts = async () => {
  ...
  try {
    ...
    // Uniquement des données réelles : aucune donnée de démonstration.
    posts.value = []
  } finally {
