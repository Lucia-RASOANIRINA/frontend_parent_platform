<template>
  <div class="w-full font-sans antialiased bg-white min-h-screen">
    ...
    <transition name="notify-slow">
      <div v-if="alert.show" 
        class="fixed top-6 right-4 left-4 md:left-auto md:right-8 md:w-80 z-[100] flex items-center gap-4 px-5 py-4 rounded-2xl shadow-2xl border border-[#F5F5DC] bg-white/95 backdrop-blur-md"
      >
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-shrink-0">
          <div 
            ...
            class="w-12 h-12 rounded-2xl bg-[#FAF7F2] border border-[#F5F5DC] flex items-center justify-center text-[#3C2A21] font-black text-sm shadow-sm transition-all duration-500 hover:border-[#6F4E37] hover:scale-105 cursor-pointer"
          >
            {{ userInitials }}
          </div>
        </div>
        ...
        <div class="flex-grow">
          <div class="relative group border-b border-[#F5F5DC] transition-all duration-500" :class="{ 'border-[#6F4E37]': isExpanded }">
            <textarea 
            ...
            <div v-if="imagePreviewUrl || pdfFile" class="mt-2 p-2 bg-[#FAF7F2] rounded-lg flex items-center gap-3">
              <div v-if="imagePreviewUrl" class="relative">
                <img :src="imagePreviewUrl" class="h-16 w-16 object-cover rounded-lg border border-[#F5F5DC]">
              ...
              <div v-if="pdfFile" class="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-[#F5F5DC]">
                ...
                <div class="relative">
                  <div class="md:hidden">
                    <button @click="showUploadMenu = !showUploadMenu" 
                      class="p-2 rounded-full bg-[#FAF7F2] text-[#3C2A21] transition-all"
                      :class="{ 'rotate-45 bg-[#3C2A21] text-white': showUploadMenu }"
                    >
    ...
    <div class="max-w-4xl mx-auto px-4 py-6">
      ...
      <div v-if="isLoadingPosts" class="space-y-6">
        <div v-for="i in 3" :key="i" class="bg-white rounded-2xl border border-[#F5F5DC] overflow-hidden shadow-sm p-4 animate-pulse">
          <div class="flex gap-3">
            <div class="w-10 h-10 rounded-full bg-[#F5F5DC]"></div>
            <div class="flex-1">
              <div class="h-4 bg-[#F5F5DC] rounded w-32 mb-2"></div>
              <div class="h-3 bg-[#F5F5DC] rounded w-24"></div>
            </div>
          </div>
          <div class="mt-4 space-y-2">
            <div class="h-3 bg-[#F5F5DC] rounded w-full"></div>
            <div class="h-3 bg-[#F5F5DC] rounded w-5/6"></div>
          </div>
          <div class="mt-4 h-40 bg-[#F5F5DC] rounded-lg"></div>
        </div>
      </div>

      <!-- Aucun post -->
      <div v-else-if="posts.length === 0" class="text-center py-12 bg-white rounded-2xl border border-[#F5F5DC]">
      ...
      <div v-else class="space-y-6">
        <div v-for="post in posts" :key="post.id" class="bg-white rounded-2xl border border-[#F5F5DC] overflow-hidden shadow-sm hover:shadow-md transition-all">
          ...
          <div class="p-4 flex justify-between items-start">
            <div class="flex gap-3">
              <div 
                ...
                class="w-10 h-10 rounded-full bg-gradient-to-br from-[#6F4E37] to-[#3C2A21] flex items-center justify-center text-white font-bold cursor-pointer hover:scale-105 transition-transform"
              >
                {{ (post.user?.nom?.charAt(0) || 'U').toUpperCase() }}
              </div>
              <div>
                <div class="font-bold text-[#3C2A21] text-sm">{{ post.user?.nom || 'Utilisateur inconnu' }}</div>
            ...
            <button 
              ...
              class="flex items-center gap-2 px-3 py-1.5 bg-[#FAF7F2] rounded-full text-xs text-[#3C2A21] hover:bg-[#6F4E37] hover:text-white transition-all"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
/* Carte utilisateur (version rose) */
.user-info-card {
  position: relative;
  width: 280px;
  background: white;
  border-radius: 1.25rem;
  box-shadow: 0 20px 35px -12px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(252, 231, 243, 0.5);
  overflow: hidden;
}
...
.user-info-arrow {
  ...
  height: 14px;
  background: white;
  border-left: 1px solid rgba(252, 231, 243, 0.8);
