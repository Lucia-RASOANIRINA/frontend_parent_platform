...
const user = utilisateurCourant() || { id: 0, nom: t('chat.invite'), role: 'EDUCATEUR' }
...
const startConversation = async (otherUser) => {
  if (!otherUser || !otherUser.id) {
    ...
    showToast(t('chat.utilisateurInconnu'), 'error')
    return
  }
  if (otherUser.id === user.id) {
    showToast(t('chat.pasVousMeme'), 'error')
    return
  }
...
const sendMessage = async () => {
  ...
  if (!receiverId) {
    showToast(t('chat.destinataireInvalide'), 'error')
    return
  }
  const messageData = {
  ...
  if (stompClient?.connected) {
    ...
    showToast(t('chat.connexionEnCours'), 'error')
    connectWebSocket()
    setTimeout(() => {
      if (stompClient?.connected) {
        stompClient.publish({ destination: '/app/chat.send', body: JSON.stringify(messageData) })
      } else {
        showToast(t('chat.envoiImpossible'), 'error')
      }
    }, 1000)
  }
  newMessage.value = ''
...
const sendFile = async (event) => {
  ...
  reader.onload = async () => {
    ...
    if (stompClient?.connected) {
      ...
      setTimeout(() => {
        if (stompClient?.connected) stompClient.publish({ destination: '/app/chat.send', body: JSON.stringify(messageData) })
        else showToast(t('chat.echecFichier'), 'error')
      }, 1000)
    }
  }
...
<style scoped>
/* ===== THÈME ROSE (harmonisé avec le header) ===== */
* {
  color: #1e293b;
}
.chat-container {
...
.message-input:focus {
  border-color: #6F4E37;
  box-shadow: 0 0 0 2px rgba(236,72,154,0.2);
}
.hidden {
  display: none;
}
