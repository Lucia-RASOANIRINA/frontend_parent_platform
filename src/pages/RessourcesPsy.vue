...
const cancelRating = async (id) => {
  try {
    await axios.delete(`${API_BASE_URL}/resources/${id}/rating`, {
      headers: { 'X-User-Id': currentUserId.value }
    })
    await fetchResources()
...
.loading-spinner { text-align: center; padding: 2rem; color: #6F4E37; display: flex; align-items: center; justify-content: center; gap: 0.5rem; }
...
.card-badge.video { background: #F5F5DC; color: #6F4E37; }
...
.fiche-preview {
  ...
  background: linear-gradient(135deg, #FBF7F1 0%, #F3EADB 100%);
  border-left: 5px solid #C19A6B; overflow-y: auto;
}
.fiche-label {
  align-self: flex-start; font-size: 0.62rem; font-weight: 800; letter-spacing: 0.12em;
  text-transform: uppercase; color: #6F4E37; background: rgba(193,154,107,0.22);
  padding: 4px 12px; border-radius: 20px;
}
.fiche-text { font-size: 0.92rem; color: #3E2C1F; line-height: 1.55; margin: 0; font-style: italic; }
.description-text {
  font-size: 0.85rem;
  color: #334155;
  line-height: 1.4;
...
.fb-like, .fb-star, .fb-comment, .fb-cancel {
  background: none;
  border: none;
  font-size: 0.75rem;
  cursor: pointer;
...
.add-comment textarea:focus { outline: none; border-color: #6F4E37; box-shadow: 0 0 0 2px rgba(8,145,178,0.1); }
...
.modal-header.stylized-header {
  background: linear-gradient(135deg, #FAF7F2 0%, #F5F5DC 100%);
...
.file-upload .file-label:hover { background: #FAF7F2; }
.file-preview {
  margin-top: 0.5rem;
  background: #FAF7F2;
  border-radius: 16px;
